...
export default function CheckoutPage() {
  ...
  const [appliedPromoCode, setAppliedPromoCode] = useState<AppliedPromoCode | null>(null); // To store applied promo code details
  const [promoCodeError, setPromoCodeError] = useState<string | null>(null);
  const [bookingError, setBookingError] = useState<string | null>(null);
  const [isProcessing, setIsProcessing] = useState(false); // To disable button during submission

  // --- Data from URL ---
  const experienceId = searchParams.get('experienceId');
  const title = searchParams.get('title');
  const slotId = searchParams.get('slotId');
  const startTimeISO = searchParams.get('startTime'); // Store as ISO for calculations
  ...
  // Ensure total isn't negative if discount is large
  ...
  const handleApplyPromoCode = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5001/api/promo-codes/apply', {
        ...
        body: JSON.stringify({ code: promoCodeInput.trim().toUpperCase() }), // Send uppercase code
      });
      ...
      setAppliedPromoCode(data.promoCode); // Store the validated promo code details
  ...
  const handlePayAndConfirm = async () => {
    ...
    try {
      const bookingData = {
        ...
        customerEmail: email,
        quantity,
        totalPrice: finalTotal, //final total
      ...
      setBookingError(error.message); // Show backend error message
  ...
  // Render null or a loading state briefly while redirecting
  if (!experienceId || !slotId || !basePriceString || !startTimeISO) {
    return (
        <>
        {/* Header (optional, or a simpler loading header) */}
        <header className="w-full bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
          <nav className="flex items-center justify-between h-[87px] max-w-[1440px] mx-auto px-[124px] py-4">
            <Link href="/" className="text-2xl font-bold text-gray-900">BookIt</Link>
          </nav>
        </header>
