...
export default function Home() {
  ...
  useEffect(() => {
    if (isLoading) return;
    if (searchTerm.trim() === "") setFilteredExperiences(allExperiences);
    else {
  ...
  }, [searchTerm, allExperiences, isLoading]);
      ...
      <header className="w-full bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
        <nav className="flex items-center justify-between h-[87px] max-w-[1440px] mx-auto px-[124px] py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="px-4 py-2 bg-[#FFD643] text-black rounded-lg font-semibold hover:bg-[#FFD643]/90 transition-colors"
            >
              Search
            </button>
          </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-pulse">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="bg-gray-200 h-72 rounded-lg border border-gray-100" />
            ))}
          </div>
          ...
          <p className="text-gray-500 text-center">No experiences found matching your search.</p>
        )}
      </main>
    </>
  );
...
function ExperienceCard({ experience }: { experience: Experience }) {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200 flex flex-col transition-transform hover:scale-[1.02]">
      ...
      <div className="p-4 bg-[#F0F0F0] flex flex-col grow">
        <div className="grow">
          <div className="flex items-baseline justify-between gap-2 mb-2">
            ...
            <span className="inline-block bg-[#D6D6D6] text-gray-800 text-xs font-medium px-2 py-1 rounded whitespace-nowrap">
              {experience.location}
            </span>
          </div>
        ...
        <div className="flex justify-between items-center mt-auto">
          <p className="text-lg font-semibold">
            <span className="text-sm font-normal text-gray-600">From </span>₹{experience.basePrice}
          </p>
          <Link
            href={`/experiences/${experience._id}`}
            className="px-4 py-2 bg-[#FFD643] text-black rounded-md text-sm font-semibold hover:bg-[#FFD643]/90 transition-colors"
          >
            View Details
          </Link>
        </div>
